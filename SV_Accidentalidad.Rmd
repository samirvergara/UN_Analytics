...
```{r}
choques_rosales<-subset(accidentalidad,
                        subset = (CLASE_ACCIDENTE=="Choque" & barrio=="Rosales"))
```
...
```{r}
accidentes_por_dia[orden_acc$ix,]
```

```{r}
plot(accidentes_por_dia$f_accidente,accidentes_por_dia$CLASE_ACCIDENTE,type="h",ylim = c(0,3),xlab="Fecha",ylab="Cantidad de accidentes",las=1,main="Choques por fecha",yaxt="n")
...
```{r}
plot(latitud,longitud,xlab="latitud",ylab="longitud")
```
...
### SV: Se agrega Festivos Colombia
```{r}
...
### 1. Corregir sesgo por festivos
### 2. Mejorar la visualización de los resultados
###   a. Usando R Básico
###   b. Usando Plottly
### 3. Pulir el reporte y subirlo a GitHub
